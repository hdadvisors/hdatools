...
|---|---|---|---|---|---|
| 0 — Groundwork | none (stays 0.2.0.9000) | `phase-0-groundwork` | CI (R-CMD-check), pha-update-2026 usage survey, pkgdown Bootstrap 5 + reference grouping + typo fixes (§1.9), consumer-pinning notes | [phase-0-groundwork.md](phase-0-groundwork.md) | **next up** |
| 1 — Consolidation | **0.3.0** | `release-0.3.0` | Tier 1 (§1.1–1.4, 1.6): safety-net tests, brand registry, one scale factory, one theme builder, colour aliases. Output provably identical. | [archive/phase-1-consolidation-0.3.0.md](archive/phase-1-consolidation-0.3.0.md) | **done** — PR open, pending merge/tag |
| 2 — Features | **0.4.0** | `release-0.4.0` | Tier 2 (§2.1–2.7): exported color vectors + accessor, designed continuous ramps + deprecations, ggplot2 4.x theme-carried palettes (floor → ≥ 4.0), VHA brand + "adding a brand" howto, CVD audit (document-only), span helper, focus palette | [phase-2-features-0.4.0.md](phase-2-features-0.4.0.md) (skeleton) | blocked by Phase 1 merge (gate: Q2, Q4, Q6–Q9) |
| 3 — Fonts | **0.5.0** | `release-0.5.0` | Tier 3.1: systemfonts/ragg migration, drop showtext/sysfonts, consumer `dev: ragg_png` guidance (§4), side-by-side render comparison in one consumer before tagging | written at phase gate | blocked by Phase 2 (gate: Q5) |
| 4 — Backlog | 0.6.0+ | per item | brand.yml emission (§3.2), vdiffr snapshots (§3.3), `use_hdatools()` (§3.4). Revisit after 0.5.0. Package split (§3.5) rejected per review. | none — deliberately unplanned | deferred |

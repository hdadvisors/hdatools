...
`theme_*()`, `scale_color_*()`/`scale_fill_*()`,
...
Before writing any code, get three things confirmed directly (never
...
Every other brand-specific symbol is a direct copy of an existing
brand’s wrapper, renamed:
...
- **`R/themes.R`**:
  ...
  calls `.brand_theme(.brands$vha, brand = "vha", ...)` with the brand’s
  own `base_size`/`base_family`/`html_adjust`/`pdf_adjust` defaults.
- **`R/scales.R`**:
  [`scale_color_vha()`](https://hdadvisors.github.io/hdatools/reference/scale_color_vha.md)/[`scale_fill_vha()`](https://hdadvisors.github.io/hdatools/reference/scale_fill_vha.md)
  (+
  [`scale_colour_vha()`](https://hdadvisors.github.io/hdatools/reference/scale_color_vha.md)
  alias) call `.scale_brand_discrete()`;
  `scale_*_vha_c()`/`scale_*_vha_b()` (+ aliases) call
  `.scale_brand_continuous()`/`.scale_brand_binned()`.
...
and (if a new font was bundled) that the `.ttf` files ship and register
offline.
...
![](adding-a-brand_files/figure-html/vha-demo-1.png)

A bare
[`theme_vha()`](https://hdadvisors.github.io/hdatools/reference/theme_vha.md)
with no `scale_*()` call also brands the plot (ggplot2 \>= 4.0’s
theme-carried palette):
...
![](adding-a-brand_files/figure-html/vha-theme-only-1.png)
